...
class Answer(models.Model):
    ...
    def check_answer_for_select(self, choices, body):
        answers = []
        # Check if body is a string that looks like a list, and parse accordingly
        if isinstance(body, str) and body.startswith("["):
            # Assuming the list is stored as a string representation of a list
            # You might need a more sophisticated parser depending on how complex the data is
            try:
                answers = eval(body)  # Be cautious with eval, ensure body is sanitized
            except:
                # Handle parsing error or log as necessary
                pass
        elif isinstance(body, str):
            # If body is a simple string, treat it as a single answer
            answers = [body]
        elif isinstance(body, list):
            # If body is already a list, use it directly
            answers = body
    
        # Sluggify the available choices for comparison
        sluggified_choices = [slugify(choice) for choice in choices]
    
        for answer in answers:
            if slugify(answer) not in sluggified_choices:
                # Construct a message indicating the invalid answer and available (original) choices
                msg = f"Invalid answer '{answer}'. Available choices: {', '.join(choices)}."
